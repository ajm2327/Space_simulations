...
pygame.display.set_caption("Vibrant Galaxy with Smooth Glowing Core")

# Colors
BLACK = (0, 0, 0)

# Function to create a smoother radial gradient surface
def create_smooth_radial_gradient(size, inner_color, outer_color):
    ...
    for y in range(size[1]):
        for x in range(size[0]):
            ...
            if distance < radius:
                # Use a quadratic easing function for smoother transition
...
core_surface = create_smooth_radial_gradient(
    ...
    (50, 50, 100, 0)       # Outer color (darker, slightly blue, fully transparent)
)
